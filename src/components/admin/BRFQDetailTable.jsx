...
const BRFQDetailTable = ({data}) => {
    
  
  const navigate = useNavigate()

  const quantity = data?.rfqId?.spreadQuantityData.reduce((acc, init)=> acc + init.quantity, 0);
  
  return (
    <>
      <div className='bg-white rounded-4xl flex flex-col gap-6 border-emerald-500 border-2 p-8 '>
        <div className="">
          <h2 className='font-semibold text-2xl'> BRFQ id: <span className='text-emerald-500'>{data?._id} </span></h2>
        </div>
        ...
        <td className='w-2/3 p-4 px-6 text-lg'> <span className='capitalize'>{data?.rfqId?.product || "N/A"}</span></td>
         
        </tr>
        <tr className='border-b-1 border-zinc-200'>
        <th  className='bg-slate-100 border-b-1 border-zinc-200 w-1/3 p-4 px-6 font-semibold text-lg text-start'>Category</th>
        <td className='w-2/3 p-4 px-6 text-lg'> <span className='capitalize'>{data?.rfqId?.category?.category || "N/A"}</span></td>
        ...
        <td className='w-2/3 p-4 px-6 text-lg'> <span className='capitalize'>{data?.rfqId?.deliverySchedule || "N/A"}</span></td>
        ...
        <td className='w-2/3 p-4 px-6 text-lg'>{data?.rfqId?.DeliveryLocation || "N/A"}</td>
         
        </tr>
        <tr className='border-b-1 border-zinc-200'>
        <th  className='bg-slate-100 border-b-1 border-zinc-200 w-1/3 p-4 px-6 font-semibold text-lg text-start'>Pin Code</th>
        <td className='w-2/3 p-4 px-6 text-lg'>{data?.rfqId?.pinCode || "N/A"}</td>
        ...
        <td className='w-2/3 p-4 px-6 text-lg '>{new Date(data?.createdAt).toLocaleDateString() || "N/A"}</td>
            ...
            <table className='w-full border-[1px] border-zinc-200'>
                <thead>
                    ...
                    <tr className='bg-slate-100 border-b-1 border-zinc-200'>
                        ...
                        <th className='p-4 text-start border-l-1 border-zinc-200'>Quantity ({data?.rfqId?.measurement})</th>
                ...
                <tbody>
                {data?.rfqId?.spreadQuantityData?.length > 0 ? data?.rfqId?.spreadQuantityData?.map((item,index)=>(
...
export default BRFQDetailTable
