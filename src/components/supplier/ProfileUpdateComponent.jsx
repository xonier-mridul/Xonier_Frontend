...
const ProfileUpdateComponent = ({ userData }) => {
  ...
  useEffect(() => {
    let filled = 0;
    const baseFields = Object.values(formData);
    filled += baseFields.filter((v) => v && v.trim() !== "").length;

    branchDetail.forEach((branch) => {
      filled += Object.values(branch).filter((v) => v && v.trim() !== "").length;
    });

    const totalFields = 6 + branchDetail.length * 3; 
          ...
          <div className="grid grid-cols-3 gap-5">
            ...
            <div className="flex flex-col gap-3">
              ...
              <select
                ...
                onChange={handleChange}
              >
                <option value="" hidden>Supplier Category</option>
