...
const SupplierOrderTable = ({ orderData, totalPages,currentPage, setCurrentPage }) => {
                  ...
                  <tbody>
                    {length>0 ?filteredData.map((item)=>(
                      <tr key={item._id} className="border-b-1 border-zinc-200">
                        <td className="p-4 px-6">{item._id || "N/A"}</td>
                        <td className="p-4 px-6 border-l-1 border-zinc-200"><span className="capitalize">{item.vrfqId?.brfqId?.rfqId?.product || "N/A"}</span></td>
                      
                        <td className="p-4 px-6 border-l-1 border-zinc-200"><span className="capitalize">{item.vrfqId?.brfqId?.rfqId?.quantity || "N/A"}</span></td>
                        <td className="p-4 px-6 border-l-1 border-zinc-200"> <span className={` ${(item.process === "dispatch" || item.process === "shipped" || item.process === "delivered")  && "bg-green-500"} capitalize bg-black px-4 py-2 rounded-lg text-sm text-white `}>{item.process || "N/A"}</span></td>
                        <td className="p-4 px-6 border-l-1 border-zinc-200"> <span className={`  capitalize bg-green-50 text-green-500 px-4 py-2 rounded-lg text-sm  `}>{new Date(item.vrfqId?.brfqId?.rfqId?.toDate).toLocaleDateString() || "N/A"}</span></td>
                        <td className="p-4 px-6 border-l-1 border-zinc-200"> <div className="flex items-center justify-center gap-3">
